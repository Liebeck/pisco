...
def patch(pipeline):
    def get_feature_names(pipeline):
        return ["number_of_local_variables_in_methods"]
    pipeline.get_feature_names = types.MethodType(get_feature_names, pipeline)


def build(stat='range'):
    pipeline = Pipeline([('transformer',
                          NumberOfLocalVariablesInMethods(stat=stat)),
                         ('min_max_scaler', MinMaxScaler())])
    patch(pipeline)
    return ('number_of_local_variables_in_methods', pipeline)


def param_grid():
    return {'union__number_of_local_variables_in_methods__transformer__stat':
            ['mean', 'variance', 'range']}


class NumberOfLocalVariablesInMethods(BaseEstimator):
